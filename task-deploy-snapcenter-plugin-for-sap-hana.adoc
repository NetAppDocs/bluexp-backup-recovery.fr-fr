...
. Copiez le contenu de *sudoer.txt* situé à : _/var/lib/docker/volumes/service-Manager[1]-2_cloudmanager_scs_cloud_volume/_data/$(sudo docker ps|grep -po « cloudmanager_scs_cloud:.*? "|sed -e's/ *$/'|cut -f2 -d":")/sc-linux-host-plugin_
...
.. Sélectionnez Plug-in host.
(Facultatif) si l'hôte n'est pas ajouté, cliquez sur *Ajouter un hôte de plug-in*.
