...
NOTE: Ces paramètres de règles sont uniquement pertinents pour la sauvegarde vers le stockage objet. Aucun de ces paramètres n'affecte vos règles de réplication ou de copie Snapshot. Des paramètres de stratégie similaires pour les instantanés et les réplications seront ajoutés ultérieurement.
