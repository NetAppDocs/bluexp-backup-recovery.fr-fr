...
Les API REST permettant de protéger les applications dans le cloud sont disponibles dans : https://[].

Vous devez obtenir le jeton utilisateur avec l'authentification fédérée pour accéder aux API REST. Pour plus d'informations sur l'obtention du jeton utilisateur, reportez-vous à la section https://["Créez un jeton utilisateur avec authentification fédérée"].
