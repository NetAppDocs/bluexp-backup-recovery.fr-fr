...
* Le mode WORM/Compliance d'un magasin d'objets est actuellement pris en charge uniquement sur Amazon S3 et StorageGRID. Il s'agit de la fonctionnalité DataLock, qui doit être gérée à l'aide des paramètres Cloud Backup, et non à l'aide de l'interface du fournisseur cloud.
...
* La priorité de restauration _High_ n'est pas prise en charge lors de la restauration des données depuis Azure vers les systèmes StorageGRID.
