...
+
** Dans AWS, https://["Amazon Athena"^] et https://["AWS Glue"^] Les ressources sont déployées dans un nouveau compartiment S3.
+
endif::aws[]
...
+
** Dans Azure, un https://["Espace de travail Azure Synapse"^] et https://["Stockage en data Lake Azure"^] sont provisionnées dans votre compte de stockage pour stocker et analyser vos données.
+
endif::azure[]
...
* Dans Google, un nouveau compartiment est déployé, et le https://["Services Google Cloud BigQuery"^] sont provisionnées au niveau compte/projet.
...
* Les sauvegardes de volume FlexGroup ne peuvent pas utiliser DataLock et la protection contre les ransomware.
