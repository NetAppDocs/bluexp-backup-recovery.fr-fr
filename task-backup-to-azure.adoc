...
Si vous créez vos propres comptes de stockage, vous devez utiliser le nom de compte de stockage « netapp-backup ».
...
* Les volumes sélectionnés doivent également avoir le même paramètre SnapLock. SnapLock Enterprise doit être activé sur tous les volumes ou SnapLock doit être désactivé. (Les volumes en mode conformité SnapLock ne sont pas pris en charge actuellement.) Vous ne pouvez pas sélectionner une combinaison de volumes verrouillés et déverrouillés.
