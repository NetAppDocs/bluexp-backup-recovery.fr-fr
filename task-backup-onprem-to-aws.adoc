...
[NOTE]
====
Si vous créez des sauvegardes dans des régions AWS GovCloud ou AWS China, vous devez modifier le nom de ressource AWS « arn » sous toutes les sections _Resource_ dans les deux ensembles d'autorisations suivantes :

* Pour les régions AWS GovCloud, vous devez remplacer « aws » par « aws-US-gov », par exemple `arn:aws-us-gov:s3:::netapp-backup-*`.
* Pour les régions d'AWS Chine, vous devez remplacer « aws » par « aws-cn », par exemple `arn:aws-cn:s3:::netapp-backup-*`.


====
.Étapes
. Vérifiez que les autorisations S3 suivantes (à partir des dernières https://docs.netapp.com/us-en/bluexp-setup-admin/reference-permissions-aws.html["Politique BlueXP"^]) Font partie du rôle IAM qui fournit au connecteur des autorisations. Si ce n'est pas le cas, consultez le https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_manage-edit.html["Documentation AWS : modification des règles IAM"].
+
[%collapsible]
====
[source, json]
...
Si vous créez vos propres compartiments, vous devez utiliser un nom de compartiment commençant par « netapp-backup ». Si vous devez utiliser un nom personnalisé, modifiez les règles utilisées dans la section Setup S3 permissions pour utiliser un ARN de ressource correspondant à votre propre préfixe de compartiment personnalisé.
...
[NOTE]
====
Si vos compartiments personnalisés se trouvent dans les régions AWS GovCloud ou AWS China, vous devez modifier le nom de ressource AWS « arn » sous toutes les sections _Resource_ des autorisations :

* Pour les régions AWS GovCloud, vous devez remplacer « aws » par « aws-US-gov », par exemple `arn:aws-us-gov:s3:::*`.
* Pour les régions d'AWS Chine, vous devez remplacer « aws » par « aws-cn », par exemple `arn:aws-cn:s3:::-*`.
