...
[role="quick-margin-para"]
Vous pouvez également configurer vos propres clés de chiffrement personnalisées pour les données au lieu d'utiliser les clés de chiffrement Google Cloud par défaut. <<Prepare Google Cloud storage as your backup target,Découvrez comment préparer votre environnement Google Cloud pour recevoir des sauvegardes ONTAP.>>
