...
* Vous avez découvert le cluster sur site et l'avez ajouté à un environnement de travail dans BlueXP. Voir https://["Découverte des clusters ONTAP"^] pour plus d'informations.
+
...
* https://["Installation d'un connecteur dans Azure"^]
* https://["Installation d'un connecteur dans vos locaux"^]
* https://["Installation d'un connecteur dans une région Azure Government"^]
+
...
+
** Connexion HTTPS sur le port 443 vers le service de sauvegarde et de restauration BlueXP et vers votre stockage objet Blob (https://["voir la liste des noeuds finaux"^])
...
** Des règles de groupes de sécurité entrants supplémentaires sont requises pour les déploiements d'Azure et d'Azure Government. Voir https://["Règles pour le connecteur dans Azure"^] pour plus d'informations.
...
+
** Pour obtenir une licence PAYGO de sauvegarde et de restauration de BlueXP, vous devez être abonné à https://["L'offre NetApp BlueXP depuis Azure Marketplace"^]. La facturation de la sauvegarde et de la restauration BlueXP s'effectue via cet abonnement.
...
.. Vous pouvez également choisir de configurer un terminal privé Azure. https://["Voir les détails sur l'utilisation d'un point d'extrémité privé"^].
+
image:screenshot_backup_onprem_azure_networking.png["Copie d'écran affichant les détails de mise en réseau lors de la sauvegarde des volumes d'un système ONTAP vers un stockage Azure Blob."]
